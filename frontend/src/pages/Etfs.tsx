...
interface EtfStats {
  ...
  flat: number
  average_nav: number
  top_gainer: EtfItem | null
  top_loser: EtfItem | null
}
...
export default function Etfs() {
  ...
  const fetchEtfs = async () => {
    ...
    setError(null)
    try {
      const url = new URL('http://localhost:8080/api/etfs')
      ...
      if (data.code === 0) {
        setEtfs(data.data.list || [])
        setTotal(data.data.total || 0)
      } else {
        setError(data.msg || '获取 ETF 数据失败')
      }
    } catch (err: any) {
  ...
  // Fetch ETF statistics
  const fetchStats = async () => {
    setStatsLoading(true)
    setStatsError(null)
    try {
      const url = new URL('http://localhost:8080/api/etfs/stats')
      url.searchParams.append('_appver', '11.17.0')
      const response = await fetch(url.toString())
      if (!response.ok) {
        throw new Error(`HTTP error: ${response.status}`)
  ...
  const columns: TableColumnsType<EtfItem> = [
    {
      title: '基金代码',
      dataIndex: 'code',
      key: 'code',
    ...
    },
    {
      ...
      sorter: true,
      align: 'right',
      render: (val: number, record: EtfItem) => (
  ...
  const openStockDetail = (record: EtfItem) => {
    setSelectedStock({
      code: record.code,
      name: record.name,
      zxj: record.zxj,
        ...
        <Row gutter={[24, 24]}>
          <Col xs={24} sm={12} md={8}>
            <Card bordered className="dashboard-card" style={{ height: '100%' }}>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '8px' }}>ETF 基金总数</div>
          ...
          <Col xs={24} md={8}>
            <Card bordered className="dashboard-card" style={{ height: '100%' }}>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '8px' }}>市场多空占比</div>
            ...
            <Card 
              ...
              title={
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--stock-up)' }}>
                  <ArrowUpOutlined />
                  <span style={{ fontWeight: 600 }}>今日领涨 ETF</span>
                </div>
              }
            >
              {stats.top_gainer ? (
            ...
            <Card 
              ...
              title={
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--stock-down)' }}>
                  <ArrowDownOutlined />
                  <span style={{ fontWeight: 600 }}>今日领跌 ETF</span>
                </div>
              }
            >
              {stats.top_loser ? (
    ...
    <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="cna-header" style={{ alignItems: 'center', marginBottom: 0 }}>
        <div className="cna-title">
          <h1>ETF 基金</h1>
          <p>沪深场内交易型开放式指数基金实时净值与行情走势列表</p>
        </div>
        <Button 
          type="default" 
          icon={<ReloadOutlined spin={loading || statsLoading} />} 
          onClick={activeTab === 'list' ? fetchEtfs : fetchStats}
      ...
      {activeTab === 'list' && (
        <>
          ...
          <Card bordered={false} styles={{ body: { padding: 0 } }} style={{ flex: 1, display: 'flex', flexDirection: 'column' }} className="dashboard-card">
            <Spin spinning={loading} tip="正在加载 ETF 基金...">
              <Table
                dataSource={etfs}
                columns={columns}
                rowKey="code"
                pagination={{
                  ...
                  showTotal: (totalCount) => `共 ${totalCount} 个 ETF 基金`,
      ...
      {activeTab === 'stats' && (
        <Spin spinning={statsLoading} tip="正在获取 ETF 统计数据...">
