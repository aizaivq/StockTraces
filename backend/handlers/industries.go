...
// GetIndustries handles GET requests to fetch industry ranking data from Tencent Finance API.
func GetIndustries(w http.ResponseWriter, r *http.Request) {
	...
	for k, vs := range query {
		if len(vs) > 0 {
			req.SetQueryParam(k, vs[0])
		}
	}

	resp, err := req.Get(url)
	if err != nil {
		log.Printf("Error calling Tencent industry API: %v", err)
		w.Header().Set("Content-Type", "application/json")
